...
const SyncStatus = ({ status, sync }: SyncStatusProps) => {
  const getStatusInfo = (status: SyncStatusType): { text: string; iconClass: string } => {
    switch (status.kind) {
      case "syncing":
        return { text: "Synchronizing...", iconClass: "sync-spinning sync-black" };
      case "idle":
        if (status.requiresSync) {
          return { text: "Needs sync", iconClass: "sync-red" };
        } else {
          return { text: "Up to date", iconClass: "sync-complete sync-black" };
        }
      case "error":
  ...
  const partialCirclePath = "M 12 2 A 10 10 0 0 1 20.66 7 A 10 10 0 0 1 20.66 17 A 10 10 0 0 1 12 22";
  ...
  return (
    ...
    >
      ...
      >
        <path 
          d={statusInfo.iconClass.includes("sync-complete") ? fullCirclePath : partialCirclePath}
          strokeWidth="2"
          strokeLinecap="round"
        />
      </svg>
